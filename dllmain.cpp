...
void InitDiscord()
{
    Discord_Initialize("1077954070033219675", NULL, 1, NULL);
    ...
    UpdatePresenceData(&DiscordPresence, NULL, "Booting up...", "game_logo", "Need for Speed: Carbon Hard Plus", NULL, NULL, std::time(nullptr));
}
            ...
            if (isPursuit)
            {
                if (type != RACE_PURSUIT_END)
                {
                    ...
                    if (!isBusted)
                    {
                        ...
                        UpdatePresenceData(&DiscordPresence, NULL, NULL, "game_logo", "Pursuit");
                    }
                    else
                    {
                        UpdatePresenceData(&DiscordPresence, "", "Busted!", "game_logo", "Pursuit");
                    }
                }
                else
                {
                    UpdatePresenceData(&DiscordPresence, "", "Pursuit Escaped!", "game_logo", "Pursuit");
                }
            }
            else
            {
                if (!IsWorldMapOpen)
                    UpdatePresenceData(&DiscordPresence, CurrentAreaName, "Currently Exploring", "game_logo", "Free Roam");
            }
        ...
        else if (strcmp(profNameBuf, "Player"))
        {
            ...
            if (*(int**)dword_BBAA8C)
            {
                ...
                case SAFEHOUSE_STARTER:
                    UpdatePresenceData(&DiscordPresence, "", "Choosing a starter car...", "game_logo", "Need for Speed: Carbon Hard Plus", "", "");
                    break;
                case CAR_LOT_MUSCLE:
                case CAR_LOT_TUNER:
                case CAR_LOT_EXOTIC:
                case CAR_LOT_MAZDA:
                    ...
                    break;
                default:
                    UpdatePresenceData(&DiscordPresence, "", "", "game_logo", "Need for Speed: Carbon Hard Plus", "", "");
                    break;
                }
            }
        }
